...
export default function BlogPost({ post }) {
  return (
    <main className='h-screen'>
      <Navbar />
      <article className=" prose max-w-2xl px-6 py-24 mx-auto space-y-12 dark:bg-gray-800 dark:text-gray-50">
        <div className="w-full mx-auto space-y-4 text-center">
          <h1 className="text-4xl font-bold leading-tight md:text-5xl mb-4">{post.title}</h1>
          <span className="text-sm dark:text-gray-400">by Admin on <time>{post.datePublished}</time> </span>
        </div>
        <div className="dark:text-gray-100" dangerouslySetInnerHTML={{ __html: post.content.html }}>
        </div>
        <div className="pt-12 border-t dark:border-gray-700">
          <div className="flex flex-col space-y-4 md:space-y-0 md:space-x-6 md:flex-row">
            <img src='/1644845701170.jpeg' alt="" className="self-center flex-shrink-0 w-24 h-24 border rounded-full md:justify-self-start 
